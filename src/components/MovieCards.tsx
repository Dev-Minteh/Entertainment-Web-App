...
// // helper: map category text -> public icon path (edit names if your files differ)
...
// }) {
//   // If caller disables trending scroller, render only a simple grid of provided items
//   if (!showTrending) {
//     return (
...
//                     {/* Play overlay shown only on hover when there is a real thumbnail */}
//                     {hasImage && (
...
//                     <button
//                       aria-label="bookmark"
//                       className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full bg-black/30 backdrop-blur-sm flex items-center justify-center hover:bg-white/20 transition"
//                     >
...
//                         <span>{item.year}</span>
//                         <span>•</span>
//                         {/* category icon replaces text when available */}
//                         {categoryIcon ? (
//                           <img src={categoryIcon} alt={item.category} className="w-3 h-3" />
//                         ) : (
...
//   // Default behavior: show trending scroller + recommended grid (homepage)
...
//               const thumbObj = item.thumbnail?.trending ?? item.thumbnail?.regular ?? {};
//               const thumbPath =
//                 (thumbObj.large as string) ??
//                 (thumbObj.small as string) ??
//                 (thumbObj.medium as string) ??
...
//                 >
//                   {/* Image */}
//                   <img src={imgSrc} alt={item.title} loading="lazy" className="w-full h-[180px] md:h-[220px] lg:h-[250px] object-cover" />
...
//                   <button
//                     aria-label="bookmark"
//                     className="absolute top-4 right-4 z-10 w-9 h-9 rounded-full bg-black/30 backdrop-blur-sm flex items-center justify-center hover:bg-white/20 transition"
//                   >
//                     <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden>
//                       <path
//                         d="M6 2h12v18l-6-3-6 3V2z"
...
//                       <span>{item.rating}</span>
//                     </div>
//                     <h3 className="text-lg md:text-2xl font-semibold leading-tight">{item.title}</h3>
//                   </div>
//                 </article>
//               );
//             })}
...
//             const thumbObj = item.thumbnail?.regular ?? {};
//             const thumbPath =
//               (thumbObj.large as string) ?? (thumbObj.medium as string) ?? (thumbObj.small as string) ?? "";
//             const imgSrc = thumbPath ? resolveImg(thumbPath) : "/placeholder.png";
//             const hasImage = !!thumbPath;
//             const categoryIcon = getCategoryIcon(item.category);

//             return (
//               <article key={`${item.title ?? "unknown"}-${item.year ?? ""}`} className="group overflow-hidden ">
//                 {/* Image */}
//                 <div className="relative">
//                   <img
//                     src={imgSrc}
...
//                     <span>{item.rating}</span>
//                   </div>
//                   <h4 className="text-sm md:text-base text-white font-semibold mt-1">{item.title}</h4>
//                 </div>
//               </article>
//             );
//           })}
              ...
              return (
                ...
                >
                  <div className="relative">
                    ...
                    >
                      <svg width="14" height="14" viewBox="0 0 24 24" fill={active ? "black" : "none"} aria-hidden>
                        <path
                          d="M6 2h12v18l-6-3-6 3V2z"
        ...
        <section>
          <div className="flex gap-6 overflow-x-auto no-scrollbar snap-x snap-mandatory scroll-smooth">
            {trendingItems.map((item) => {
              const thumbObj = item.thumbnail?.trending ?? item.thumbnail?.regular ?? {};
              const thumbPath =
                (thumbObj.large as string) ?? (thumbObj.small as string) ?? (thumbObj.medium as string) ?? "";
              const imgSrc = thumbPath ? resolveImg(thumbPath) : "/placeholder.png";
              const hasImage = !!thumbPath;
              const categoryIcon = getCategoryIcon(item.category);
              const active = isBookmarked(item.title);
              ...
              return (
                ...
                >
                  ...
                  >
                    <svg width="18" height="18" viewBox="0 0 24 24" fill={active ? "black" : "none"} aria-hidden>
                      <path
                        d="M6 2h12v18l-6-3-6 3V2z"
                        stroke={active ? "black" : "white"}
                        strokeWidth="1.2"
                  ...
                  <div className="absolute left-6 bottom-6 z-10 text-white">
                    <div className="text-xs text-gray-200/75 mb-2 flex items-center gap-2">
                      ...
                      <span>•</span>
                      {categoryIcon ? (
                        <img src={categoryIcon} alt={item.category} className="w-4 h-4" />
                      ) : (
                        <span>{item.category}</span>
                      )}
                      <span>•</span>
